...
# This script performs some preparation works for CSJ demo.
# Please put s5_demo directory that include this script under egs/csj/

[ ! -e ../../../egs ] && echo "Error: Please put s5_demo directory under egs/csj/." && exit 1;

echo "Modify some scripts and make link files for CSJ demo..."
...
if [ -e steps ] ;then
    echo -e "#!/bin/bash\n# 2016 CSJ Kaldi light version (by TokyoTech and MERL): GPU is turned off." > 08_dnn/train.sh
    sed 's:\-\-use\-gpu\=yes:\-\-use\-gpu\=no:g' steps/nnet/train.sh | grep -v "#!/bin/bash" >>08_dnn/train.sh || exit 1;
    echo -e "#!/bin/bash\n# 2016 CSJ Kaldi light version (by TokyoTech and MERL): GPU is turned off." > 08_dnn/pretrain_dbn.sh
    ...
    echo -e "#!/bin/bash\n# 2016 CSJ Kaldi light version (by TokyoTech and MERL): Do not generate lattices." > $nolats

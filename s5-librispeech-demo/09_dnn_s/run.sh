...
steps/nnet/align.sh --nj 1 --cmd "$train_cmd" \
  $data_fmllr/train_nodup ../data/lang_nosp $srcdir dnn5b_pretrain-dbn_dnn_ali || exit 1;
# denlats
steps/nnet/make_denlats.sh --nj 1 --sub-split 1 --cmd "$decode_cmd" --config conf/decode_dnn.config \
  --acwt $acwt $data_fmllr/train_nodup ../data/lang_nosp $srcdir dnn5b_pretrain-dbn_dnn_denlats || exit 1;

# Re-train the DNN by 1 iteration of sMBR
./train_mpe.sh --cmd "$cuda_cmd" --num-iters 1 --acwt $acwt --do-smbr true --skip_cuda_check true \
  $data_fmllr/train_nodup ../data/lang_nosp $srcdir dnn5b_pretrain-dbn_dnn_ali dnn5b_pretrain-dbn_dnn_denlats $dir || exit 1;
...
for eval_num in `seq 1`; do
    ...
    if $skip_make_lattice ; then
        ...
        cp -r dnn5b_pretrain-dbn_dnn_smbr/lat.1.gz $dir/decode_eval${eval_num}_librispeech/lat.1.gz

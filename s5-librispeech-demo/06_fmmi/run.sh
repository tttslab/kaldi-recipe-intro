...
time steps/train_diag_ubm.sh --silence-weight 0.5 --nj 1 --cmd "$train_cmd" \
  200 ../data/train_nodup ../data/lang_nosp ../04_sat/exp/tri4_ali_nodup exp/tri4_dubm

time steps/train_mmi_fmmi.sh --learning-rate 0.005 --boost 0.1 --cmd "$train_cmd" \
  ../data/train_nodup ../data/lang_nosp ../04_sat/exp/tri4_ali_nodup exp/tri4_dubm \
...
for eval_num in `seq 1`; do
	...
	decode_dir=exp/tri4_fmmi_b0.1/decode_eval${eval_num}_it${iter}_librispeech

	time steps/decode_nolats.sh --nj 1 --cmd "$decode_cmd" --iter $iter \
	    --transform-dir ../04_sat/exp/tri4/decode_eval${eval_num}_librispeech \
	...
	zcat $decode_dir/words.*.gz | copy-int-vector ark:- ark,t:- >$decode_dir/nolats_int.hyp
	 int2sym.pl -f 2: ../data/lang_nosp_test_tgsmall/words.txt  $decode_dir/nolats_int.hyp | local/wer_hyp_filter >$decode_dir/nolats_stem.hyp
